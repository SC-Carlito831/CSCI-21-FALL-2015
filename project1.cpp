 ...
 * last modified
 *
 * References: cplusplus.com (islower and isupper functions calls)
 */
#include <cassert>
#include <climits>
#include <iostream>
 ...
 * @param theString a string containing characters to be counted
 * @param alpha an integer that counts the number of alphabetic chars in 
 * 		the string
 * @param num an integer that counts the number of numeric chars in the string
 */
void countCharacters (string theString, int& alpha, int& num);

/*
 * Capitalize/lowercase every other character in a string -- first 
 * character of the string should be capitalized, the second character 
 * lowercase, etc.
 * @param theString a string whose chars will be converted to lower & upper case
 * @return a string that has every other char in uppercase, starting with the
 * 		first one
 */
 ...
 * assume that the parameter will never have multiple spaces back-to-back, 
 * and will never begin or end with spaces.
 * @param theString a string containing words to be counted
 * @return an integer containing the number of words in the string theString
 */
int countWords (string theString);

/*
 * Compute the average of the values in an array; assume the function will 
 * never receive an array of size 0.
 * @param values an integer array
 * @param arraySize the size of the integer array values
 * @return an integer containing the average of all the integers in array
 * 		values
 */
int computeAverage (int values [], int arraySize);

/*
 * Identify the smallest value in an array; assume the function will never 
 * receive an array of size 0.
 * @param values an integer array
 * @param arraySize the size of the integer array values
 * @return an integer containing the smallest value in the array values
 */
int findMinValue (int values [], int arraySize);

/*
 * Identify the largest value in an array; assume the function will never 
 * receive an array of size 0
 * @param values an integer array
 * @param arraySize the size of the integer array values
 * @return an integet containing the largest value in the array values
 */
...
string upAndDown (string theString) {
	if (!theString.empty()) {
		for (int i = 0; i < theString.length(); i++) {
			if((i % 2 == 0) && (islower(theString.at(i))))
				theString.at(i) = toupper(theString.at(i));
			else if ((i % 2 == 1) && (isupper(theString.at(i))))
