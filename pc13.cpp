 ...
 * modified on 30 sep
 *
 * References: cplusplus.com (searching for a word in a string), stackoverflow.com
 * (How to search/find  and replace a string)
 */
#include <cassert>
#include <cstdlib>
#include <iostream>
...
bool findWord (string* theString, string theWord) {
	if (theString == NULL) {
		...
		size_t pos = 0;
		if ((pos = theString->find(theWord)) != string::npos)
			return true;
		else
			return false;
	}
...
bool replaceWord (string* theString, string oldWord, string newWord) {
	if (theString == NULL) {
		...
		size_t pos = 0;
		if ((pos = theString->find(oldWord, pos)) != string::npos) {
